...
int main(int argc, char* argv[])
{
    ...
    Initdata_float(A_origin, x_origin, y_origin, padding_m, padding_n);
    ...
    for(int i=0; i<5000+warmup; i++){
        ...
        // sprintf(info, "dense");
        if(mpirank == 0){
            errorstat[i-warmup] = netlib_error_float(y, y_origin, padding_m);
            // if(i % 100 == 0)
            // perfoutput(maxt, padding_m, padding_n, info, A_origin);
            timestat[i-warmup] = maxt;
        }
    }
    double bytes = sizeof(real_t) * (m * n + m + n);
    char info[200];
    sprintf(info, "log");
    writestat("testmilan.log", info, timestat, errorstat, bytes, 5000);
    MPI_Finalize();
    return 0;
}
