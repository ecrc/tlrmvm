...
protected:
    ...
    void SetUp() {
        ...
        size_t post = 0;
        for(int i=0; i<STREAMSIZE; i++){
            post += streamexecsize[i];
            
            int Ntlocal = streamexecsize[i];
            Matrix<int> Rmat_subset = Rmat.Block({0,Mtglobal}, {prev, post});
            vector<int> colrsum_subset = Rmat_subset.ColSum();
            vector<size_t> tmpAvMs;
            vector<size_t> tmpAvKs;
            vector<size_t> tmpAvNs;
            vector<size_t> tmpAuMs;
            vector<size_t> tmpAuKs;
            vector<size_t> tmpAuNs;
            for(int j=0; j<Ntlocal; j++){
                tmpAvMs.push_back((size_t)colrsum_subset[j]);
                tmpAvKs.push_back(nb);
                tmpAvNs.push_back(1);
            }
            AvMs_graph.push_back(tmpAvMs);
            AvKs_graph.push_back(tmpAvKs);
            AvNs_graph.push_back(tmpAvNs);
            ...
            CaluclateTotalElements(AvMs_graph[i], AvKs_graph[i], AvNs_graph[i], 
            Avtotalelems_graph[i], xtotalelems_graph[i], yvtotalelems_graph[i]);
            size_t DataAvJumpingelem = 0;
            for(int dvi=0; dvi < prev; dvi++) DataAvJumpingelem += nb * colrsum[dvi];

            memcpy(hAv_graph[i], DataAv + DataAvJumpingelem, sizeof(float) * Avtotalelems_graph[i]);
            ...
            // copy hx to hx_graph[i]
            hx_graph[i] = new float[streamexecsize[i] * nb];
            memcpy(hx_graph[i], hx + prev * nb, sizeof(float)*streamexecsize[i] * nb);
            ...
            // phase 2
            size_t localranksum = Rmat_subset.Sum();
            GetDeviceMemory(&offsetphase2_graph[i], (size_t)localranksum);
            tlrmvm::CalculatePhase2Offset(&offsetphase2_h_graph[i], Mtglobal, Ntlocal, Rmat_subset);
            cudatlrmat::CopyDataB2HD(offsetphase2_graph[i], offsetphase2_h_graph[i], localranksum);
            // phase 3
            vector<int> rowrsum_subset = Rmat_subset.RowSum();
            for(int j=0; j<Mtglobal; j++){
                tmpAuMs.push_back(nb);
                tmpAuKs.push_back( (size_t)rowrsum_subset[j] );
                tmpAuNs.push_back(1);
            }
            AuMs_graph.push_back(tmpAuMs);
            AuKs_graph.push_back(tmpAuKs);
            AuNs_graph.push_back(tmpAuNs);
            ...
            CaluclateTotalElements(AuMs_graph[i], AuKs_graph[i], AuNs_graph[i], 
            Autotalelems_graph[i], yutotalelems_graph[i], ytotalelems_graph[i]);
            ...
            // copy STREAMSIZE Y POINTER TO DEVICE
            ...
            prev += streamexecsize[i];
        }
        CopyDataB2HD(y_graph_ondevice, y_graph, STREAMSIZE);
        // at this stage, hAu is not inited because we need to allocate all 
        // memory to finish the copying.
        
        size_t DataAuJumpingelem = 0;
        float **Austreamoffset;
        Austreamoffset = new float *[STREAMSIZE];
        for(int si=0; si < STREAMSIZE; si++){
            Austreamoffset[si] = hAu_graph[si];
        }
        vector<vector<int>> rowsumcollect;
        prev = post = 0;
        for(int si=0; si < STREAMSIZE; si++){
            post += streamexecsize[si];
            rowsumcollect.push_back(Rmat.Block({0,Mtglobal}, {prev, post}).RowSum());
            prev += streamexecsize[si];
        }
        prev = post = 0;
        for(int si=0; si < STREAMSIZE; si++){
            post += streamexecsize[si];
            granksum_graph.push_back(Rmat.Block({0,Mtglobal}, {prev, post}).Sum());
            prev += streamexecsize[si];
        }
        prev = post = 0;
        for(int mi=0; mi < Mtglobal; mi++){
            size_t curcoloffset = 0;
            size_t curstreamrowsum = 0;
            for(int si=0; si<STREAMSIZE; si++){
                memcpy(Austreamoffset[si], DataAu + DataAuJumpingelem + curcoloffset*nb, 
                rowsumcollect[si][mi] * nb * sizeof(float));
                curcoloffset += rowsumcollect[si][mi];
                Austreamoffset[si] += rowsumcollect[si][mi] * nb;
            }
            DataAuJumpingelem += rowrsum[mi] * nb;
        }
        delete[] Austreamoffset;
        // Now we finish hAu copy. Then copy to device.
        for(int i=0; i < STREAMSIZE; i++){
        ...
        GetDeviceMemory(&yfinal_graph, paddingM);
        cout << "DataAV[0]" << DataAv[0]  << " x"  << hx[0] << endl;
        delete[] DataAv; 
        delete[] DataAu;
        delete[] DataR;
