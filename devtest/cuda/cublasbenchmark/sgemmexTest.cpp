...
BENCHMARK_DEFINE_F(SeismicFixture, FloatinFloatoutTest)(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        CUBLASCHECK(cublasgemmex(handle, CUBLAS_OP_N, CUBLAS_OP_N, originM, 2, originN, 
        &alpha, floatA_r, CUDA_R_32F, originM, floatx_ri, CUDA_R_32F, originN, 
        &beta, floaty_r, CUDA_R_32F, originM, CUBLAS_COMPUTE_32F, CUBLAS_GEMM_DEFAULT));

        CUBLASCHECK(cublasgemmex(handle, CUBLAS_OP_N, CUBLAS_OP_N, originM, 2, originN, 
        &alpha, halfA_i, CUDA_R_32F, originM, floatx_ri, CUDA_R_32F, originN, 
