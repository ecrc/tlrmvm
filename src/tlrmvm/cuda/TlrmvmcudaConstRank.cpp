...
#include "TlrmvmcudaConstRank.hpp"
...
namespace cudatlrmvm
{
    ...
    TlrmvmcudaConstRank<HostType, DeviceType>::TlrmvmcudaConstRank() {}

    template<typename HostType, typename DeviceType>
    TlrmvmcudaConstRank<HostType, DeviceType>::TlrmvmcudaConstRank(TlrmvmConfig tlrmvmconfig)
            :config(tlrmvmconfig)
    {
        transpose = false;
        conjugate = false;
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType, DeviceType>::UpdateConfig(TlrmvmConfig &tlrmvmconfig)
    {
//        transpose = false;
//        conjugate = false;
//        init_alpha_beta(alpha, beta);
//        tlrmvmcpu->UpdateConfig(tlrmvmconfig);
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::setX(HostType * xvector, size_t xlength){
        tlrmvmcpu->setX(xvector, xlength);
        tlrmvmcpu->TryConjugateXvec();
    }

    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::TryConjugateXvec() {
        // no transpose logic
        tlrmvmcpu->TryConjugateXvec();
        CopyDataB2HD((HostType*)this->cudap1ptrs.x, tlrmvmcpu->p1ptrs.x, tlrmvmcpu->xmat.Shape()[0]);
    }
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::TryConjugateResults() {
        if(!conjugate) return;
        if(transpose){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::StreamInit(int streamsize){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::StreamDestroy(){
        cublasDestroy(cublashandle);
        cudaStreamDestroy(stream);
    }

    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::MemoryInit(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::MemoryFree(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase1(){
        cudaDeviceSynchronize();
        cudaDeviceSynchronize();
    }

    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase1Transpose(){
        cudaDeviceSynchronize();
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase1GetMembuffer(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase1GetMembufferTranspose()
    {
        int batchsize = cudap1ptrs.Ms.size();
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::AllocatePhase1Buffer(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::AllocatePhase1BufferTranspose(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase1CopyData(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase1CopyDataTranspose(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase2(){
        cudaDeviceSynchronize();
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase2Transpose(){
        cudaDeviceSynchronize();
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase2Prepare(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase2PrepareTranspose(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase3(){

    }
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase3Transpose(){

    }

    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase3GetMembuffer(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase3GetMembufferTranspose(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::AllocatePhase3Buffer(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::AllocatePhase3BufferTranspose(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase3CopyData(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::Phase3CopyDataTranspose(){
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::MVMTranspose()
    {
        cudaDeviceSynchronize();
        for(int i=0; i<config.Ntg; i++){
            if(cudap1transptrs.Ms[i] != 0){
                cublasgemv(cublashandle, CUBLAS_OP_T,
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType,DeviceType>::MVMNoTranspose()
    {
        cudaDeviceSynchronize();
        // batch ops
        cublasgemmbatched(cublashandle, CUBLAS_OP_N, CUBLAS_OP_N,
...
//        }
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType, DeviceType>::CopyBackResults()
    {
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType, DeviceType>::MVM() {
        if(transpose){
            MVMTranspose();
        }else{
            MVMNoTranspose();
    ...
    template<typename HostType, typename DeviceType>
    void TlrmvmcudaConstRank<HostType, DeviceType>::SetTransposeConjugate(bool transpose, bool conjugate) {
    ...
    template class TlrmvmcudaConstRank<float, float>;
    template class TlrmvmcudaConstRank<double, double>;
    template class TlrmvmcudaConstRank<complex<float>, cuComplex>;
    template class TlrmvmcudaConstRank<complex<double>, cuDoubleComplex>;
