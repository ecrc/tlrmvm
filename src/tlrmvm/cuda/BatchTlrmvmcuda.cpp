...
namespace cudatlrmvm
{
    ...
    BatchTlrmvmcuda<HostType, DeviceType>::BatchTlrmvmcuda(vector<TlrmvmConfig> tlrmvmconfigvec)
    ...
    template<typename HostType, typename DeviceType>
    BatchTlrmvmcuda<HostType, DeviceType>::BatchTlrmvmcuda(){}
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::StreamInit(int streamsize){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::MemoryInit(){
...
#pragma omp parallel for default(none)
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType, DeviceType>::SetTransposeConjugate(bool transpose, bool conjugate){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::setX(HostType * xvector, size_t xlength){
        int offset = 0;
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::TryConjugateXvec() {
        for(int bi=0; bi<batchsize; bi++){
            // no transpose logic
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase1(){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase1Transpose(){
        cudaDeviceSynchronize();
        for(int bi=0; bi < batchsize; bi++){
            for(int i=0; i<config_vec[bi].Ntg; i++){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase1Prepare() {
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase1PrepareTranspose() {
        for(int bi=0; bi < batchsize; bi++){
            int curbatch = cudap1ptrs_vec[bi].Ms.size();
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase2(){
        cudaDeviceSynchronize();
        for(int bi=0; bi<batchsize; bi++){
            phase2_nosplit<DeviceType>(cudap1ptrs_vec[bi].y, d_phase2mapping_vec[bi],
                                       cudap3ptrs_vec[bi].x,
                                       config_vec[bi].workmatgranksum, streamptr[bi % stream_size]);
        }
        cudaDeviceSynchronize();
    }
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase2Transpose(){
        cudaDeviceSynchronize();
        for(int bi=0; bi<batchsize; bi++){
            phase2_nosplit<DeviceType>(cudap1transptrs_vec[bi].y, d_phase2mapping_transpose_vec[bi],
                                       cudap3transptrs_vec[bi].x, config_vec[bi].workmatgranksum,
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase2Prepare(){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase2PrepareTranspose(){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase3(){
        cudaDeviceSynchronize();
        for(int bi=0; bi<batchsize; bi++){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase3Transpose(){
        cudaDeviceSynchronize();
        for(int bi=0; bi<batchsize; bi++){
            for(int i=0; i<config_vec[bi].Mtg; i++){
                cublasgemv(cublashandleptr[i%stream_size], CUBLAS_OP_T,
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase3Prepare() {
        for(int bi=0; bi<batchsize; bi++){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::Phase3PrepareTranspose() {
        for(int bi=0; bi<batchsize; bi++){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::MVM_MultiGraph(){
        if(transpose){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::MVM_MultiGraphTranspose(){
        ...
        for(int bi=0; bi<batchsize; bi++){
            if(!graphCreated[bi]){
                ...
                phase2_nosplit<DeviceType>(cudap1transptrs_vec[bi].y,
                                           ...
                                           config_vec[bi].workmatgranksum, streamptr[0]);
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::MVM_MultiGraphNoTranspose()
    {
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::MVM_SingleGraph()
    {
        if(transpose){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::MVM_SingleGraphTranspose()
    {
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::MVM_SingleGraphNoTranspose()
    {
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::TryConjugateResults() {
//        if(!conjugate) return;
        if(transpose){
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType, DeviceType>::CopyBackResults()
    {
        size_t offset = 0, origin = 0;
        for(int bi=0; bi<batchsize; bi++){
            // use cpu pointers to send output
            if(transpose){
                CopyDataB2HD(cpuinstvec[bi]->p1transptrs.y, (HostType*)cudap1transptrs_vec[bi].y, cpuinstvec[bi]->config.workmatgranksum);
                CopyDataB2HD(cpuinstvec[bi]->p3transptrs.x, (HostType*)cudap3transptrs_vec[bi].x, cpuinstvec[bi]->config.workmatgranksum);
                CopyDataB2HD(cpuinstvec[bi]->p3transptrs.y, (HostType*)cudap3transptrs_vec[bi].y, cpuinstvec[bi]->config.originM);
...
//                memcpy(cpuinstvec[bi]->p3transptrs.y, &alpha, sizeof(HostType));
                ...
                CopyDataB2HD(cpuinstvec[bi]->p1ptrs.y, (HostType*)cudap1ptrs_vec[bi].y, cpuinstvec[bi]->config.workmatgranksum);
                CopyDataB2HD(cpuinstvec[bi]->p3ptrs.x, (HostType*)cudap3ptrs_vec[bi].x, cpuinstvec[bi]->config.workmatgranksum);
                CopyDataB2HD(cpuinstvec[bi]->p3ptrs.y, (HostType*)cudap3ptrs_vec[bi].y, cpuinstvec[bi]->config.originM);
                origin = cpuinstvec[bi]->config.originM;
    ...
    template<typename HostType, typename DeviceType>
    void BatchTlrmvmcuda<HostType,DeviceType>::MemoryFree(){
        for(int bi=0; bi<batchsize; bi++){
    ...
    template class BatchTlrmvmcuda<float, float>;
    template class BatchTlrmvmcuda<double, double>;
    template class BatchTlrmvmcuda<complex<float>, cuComplex>;
    template class BatchTlrmvmcuda<complex<double>, cuDoubleComplex>;
